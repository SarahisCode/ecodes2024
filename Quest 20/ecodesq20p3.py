with open("ecodesq20p3input.txt", "r") as a:
...
DIRECTIONS = (Coord(1,0), Coord(0,1), Coord(0, -1))
def find_new_info(dir, new_pos, grid, altitude):
    new_altitude = altitude + height_change(grid[new_pos.x%grid_height][new_pos.y])
...
def advance(info, grid):
    ...
    for direction in DIRECTIONS:
        if direction.x != -info[0].x or direction.y != -info[0].y:
            ...
            if valid(new_pos):
                char_at = grid[new_pos.x%grid_height][new_pos.y]
                if char_at != "#":
...
def valid(pos):
    if 0 <= pos.x and 0 <= pos.y < grid_width:
        return True
    return False
...
gprint(blop, max_altitude, best_other_info[0], best_other_info[1])
print(time.time()-old)
